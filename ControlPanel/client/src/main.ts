...
document.querySelector<HTMLDivElement>("#page_title h1")!.innerText = await fetch_string_from_api("room_name");
...
async function fetch_string_from_api(endpoint: string): Promise<string> {
  let max_retries = 10;
  let response = null;
  while (max_retries > 0) {
    try {
...
async function updateTimer(): Promise<void> {
  ...
  if (timerElement === null) {
    ...
    timerElement.innerText = timerText; // Update the timer text
    ...
    } else if (timerText === "ERROR") {
      timerElement.classList.add("error");
    } else if (timerText === "STOPPED") {
      timerElement.classList.add("stopped");
    } else if (timerText === "PAUSED") {
