...
document.querySelector<HTMLDivElement>("title")!.innerText =
  "Control Panel for TITLE TODO";
...
document.querySelector<HTMLDivElement>("#page_title")!.innerHTML = `
  <h1>CONTROL</h1>
  <div class="kana">
...
document.querySelector<HTMLDivElement>("#navigation")!.innerHTML = `
  <button id="nav_control" class="nav-button nav-act">Control Panel</button>
...
async function updateTimer() {
  try {
    ...
    const timeRemaining = await response.text(); // Fetch the response as a string
    const timerElement = document.getElementById("timer");
    if (timerElement === null) {
      throw new Error("Timer element not found");
    } else {
      timerElement.innerText = timeRemaining; // Update the timer text

      timerElement.classList.remove("ready", "paused", "stopped", "error", "mtquart");

      if (timeRemaining === "READY") {
          timerElement.classList.add("ready");
      } else if (timeRemaining === "ERROR") {
          timerElement.classList.add("error");
      } else if (timeRemaining === "STOPPED") {
          timerElement.classList.add("stopped");
      } else if (timeRemaining === "PAUSED") {
