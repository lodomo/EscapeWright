...
# If the switch is normally closed, the lock will open when the switch is open 
class LockNSwitchNC(Role):
...
# If the switch is normally open, the lock will open when the switch is closed 
class LockNSwitchNO(Role):
    def __init__(self, logger = None, switch_pin = 21, maglock_pin = 17):
        self.observer = None    # Inherited from Role
        self.status = None      # Inherited from Role
        self.running = False    # Inherited from Role
