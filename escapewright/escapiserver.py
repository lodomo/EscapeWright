...
#     Purpose: API for the Control Console to the Client Pis.
#     Updated: 01 APR 2023
# Description:
#
###############################################################################
...
class EscapiServer:
    def __init__(
        ...
        # Add this server as an observer to the role
        self.role.set_observer(self)
        self.ip = ip  # IP of the Pi
        self.transmitter = EscapiTransmitter(
            self.name, control_ip, logger, self.port)
    ...
    def define_routes(self):
        ...
        self.log(
            f"index, relay, status, start, override, reset, stop, reboot, logs", "DEBUG"
        )
            ...
            try:
                if message:
                    ...
                    if self.role.process_message(message):
                        ...
                        self.log(
                            f"Relay: {message}, Event Not Activated", "INFO")
        ...
        def status():
            self.log(
                f"Status Requested, current status is: {self.role.status}", "DEBUG"
            )
    ...
    def task_responses(self, function, option, request_name):
        ...
        self.log(
            f"{ request_name } Requested, current status is: {self.role.status}", "INFO"
        )
